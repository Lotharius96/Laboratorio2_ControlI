...
{
 43,  45,  47,  48,  50,  52,  53,  54,  56,  56,  57,  58,  58,  58,  58,  57,  57,  56,
 55,  54,  52,  51,  49,  48,  46,  44,  42,  40,  38,  37,  35,  34,  32,  31,  30,  29,
 29,  28,  28, 28,  28,  29,  30,  30,  32,  33,  34,  36,  38,  39,  41
...
void setup() {
 ...
 // const int delayMilis = delayFreq[12];
...
void loop() {
  const int delayMilis = delayFreq[5]; //Según el valor seleccionado, se establece el delay
  const int pinNumber = 11;
  const int pin_giro1 = 7;
  const int pin_giro2 = 8;
 ...
 // delay(delayMilis); //retardo según la frecuencia deseada
  ...
  if (pos_motor != pos_tmp) {
    ...
    if (angular_speed < 20 && angular_speed > -20) {
      y = 0.904 * y + 0.09654 * angular_speed;
    } else {
      y = y;
    }
   ...
   // Serial.print("\t");
   // Serial.print(analogRead(A0)*0.0048825,3); 
   // Serial.print("\t");
   // Serial.println(input_val,3);

    //}
    }
   
  }
    Serial.print((time_back) / 1.0, 2);
    Serial.print("\t");
    
    double analog_value=pwmVal*5*7.1343/256.0-1.99;
    Serial.print(analog_value/1.0,2);
    Serial.print("\t");
    Serial.println(y,3);
    //Serial.print((time_ref) / 10.0, 2);
    //Serial.print("\t");
   // Serial.println(pwmVal, 3);
  if (millis() - time_back >= delayMilis) {
    time_back = 0;
    pwmFloats++;
